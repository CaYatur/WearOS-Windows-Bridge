...
public sealed class BridgeHost : IDisposable
{
    ...
    public BridgeHost(BridgeConnectionHandler handler, ConnectionStatus status, byte[] key){_handler=handler;_status=status;_bluetooth=new(handler);_pairing=new(key);}

    public void Start(){_tcp.Start();_discovery.Start();_pairing.Start(); _=RunTcpAsync(); _=RunBluetoothAsync();}
    ...
    public void Dispose(){_shutdown.Cancel();_tcp.Stop();_discovery.Dispose();_pairing.Dispose();_bluetooth.Dispose();_shutdown.Dispose();}
}
